...
fn calibrate_offset_below_floor_err() {
    ...
    let err = driver.set_offset(Fast, -65).unwrap_err();
...
fn calibrate_offset_above_ceiling_err() {
    ...
    let mut i2c = I2cMock::new(&expectations);
    let mut driver = Pcf8523::new(&mut i2c, Pcf8523T {}).unwrap();
    let err = driver.set_offset(Fast, 64).unwrap_err();
    assert_eq!(err, InvalidArgument);
    i2c.done();
}
...
fn calibrate_offset_fast_ok() {
    ...
    let mut i2c = I2cMock::new(&expectations);
    let mut driver = Pcf8523::new(&mut i2c, Pcf8523T {}).unwrap();
    driver.set_offset(Fast, 15).unwrap();
    i2c.done();
}
...
fn calibrate_offset_slow_ok() {
    ...
    let mut i2c = I2cMock::new(&expectations);
    let mut driver = Pcf8523::new(&mut i2c, Pcf8523T {}).unwrap();
    driver.set_offset(Slow, -15).unwrap();
    i2c.done();
}
...
#[test]
fn enable_correction_interrupt_invalid_state_err() {
    let expectations = [
        I2cTransaction::read(PCF8523_I2C_ADDRESS, [0b0].to_vec()),
...
fn new_err() {
    ...
    let mut i2c = I2cMock::new(&expectations);
    match Pcf8523::new(&mut i2c, Pcf8523T {}) {
...
fn set_clkout_max_ok() {
    ...
    let mut i2c = I2cMock::new(&expectations);
    let mut driver = Pcf8523::new(&mut i2c, Pcf8523T {}).unwrap();
    driver.set_clkout(ClkOut::Frequency32_768Hz).unwrap();
    i2c.done();
}

#[test]
fn set_clkout_min_ok() {
    let expectations = [
        I2cTransaction::read(PCF8523_I2C_ADDRESS, [0b0].to_vec()),
        i2c_reg_read(PCF8523_TMR_CLKOUT_CTRL, 0b1100_0000),
    ...
    let mut i2c = I2cMock::new(&expectations);
    let mut driver = Pcf8523::new(&mut i2c, Pcf8523T {}).unwrap();
    driver.set_clkout(ClkOut::Frequency0Hz).unwrap();
    i2c.done();
}
...
#[test]
fn start_second_timer_pulsed_ok() {
    let expectations = [
        I2cTransaction::read(PCF8523_I2C_ADDRESS, [0b0].to_vec()),
    ...
    let mut i2c = I2cMock::new(&expectations);
    let mut driver = Pcf8523::new(&mut i2c, Pcf8523T {}).unwrap();
    driver.start_second_timer(Pulsed).unwrap();
    i2c.done();
}

#[test]
fn start_second_timer_permanent_ok() {
    let expectations = [
        I2cTransaction::read(PCF8523_I2C_ADDRESS, [0b0].to_vec()),
    ...
    let mut i2c = I2cMock::new(&expectations);
    let mut driver = Pcf8523::new(&mut i2c, Pcf8523T {}).unwrap();
    driver.start_second_timer(PermanentlyActive).unwrap();
    i2c.done();
}
...
fn stop_second_timer_ok() {
    ...
    let mut i2c = I2cMock::new(&expectations);
    let mut driver = Pcf8523::new(&mut i2c, Pcf8523T {}).unwrap();
    driver.stop_second_timer().unwrap();
    i2c.done();
}
